...
extern counter_t ghb_updates;
int ghb_enable = 0;
static int ghb_degree = 4;
/* ---- Step 4.3: GHB + Index Table (train only) ---- */
#ifndef GHB_SIZE
...
struct it_entry_t {
  md_addr_t pc_tag;
  int head;
  unsigned int head_stamp;
  int valid;
};

static struct ghb_entry_t ghb[GHB_SIZE];
static struct it_entry_t itab[IT_SIZE];

static int ghb_head = 0;
static unsigned int ghb_stamp = 1;

/* stats */
static counter_t ghb_hook_misses = 0;
static counter_t ghb_trains = 0;
static counter_t it_hits = 0;
static counter_t it_misses = 0;
static counter_t ghb_overwrites = 0;
static counter_t ghb_lookups = 0;
static counter_t ghb_chain2_ok = 0;
static counter_t ghb_stride_found = 0;
static counter_t ghb_pref_candidates = 0;
static counter_t ghb_pref_issued = 0;
static counter_t ghb_pref_skip_no_ul2 = 0;

static inline unsigned int
ghb_hash_pc(md_addr_t pc)
{
  unsigned int x = (unsigned int)(pc >> 2);
  x ^= (x >> 11);
  x *= 2654435761u;
  return x;
}
...
ghb_init(void)
{
  ...
  for (i = 0; i < GHB_SIZE; i++) {
    ghb[i].block_addr = 0;
    ghb[i].prev = GHB_INVALID_INDEX;
    ghb[i].stamp = 0;
  }

  for (i = 0; i < IT_SIZE; i++) {
    itab[i].pc_tag = 0;
    itab[i].head = GHB_INVALID_INDEX;
    itab[i].head_stamp = 0;
    itab[i].valid = 0;
  }
}
...
static void
ghb_train(md_addr_t pc, md_addr_t block_addr)
{
  if (!ghb_enable) return;
  unsigned int it_idx = ghb_hash_pc(pc) % IT_SIZE;
  int prev = GHB_INVALID_INDEX;

  if (itab[it_idx].valid && itab[it_idx].pc_tag == pc) {
    int h = itab[it_idx].head;
    if (h != GHB_INVALID_INDEX && ghb[h].stamp == itab[it_idx].head_stamp) {
      prev = h;
      it_hits++;
    } else {
      it_misses++;
      prev = GHB_INVALID_INDEX;
    }
  } else {
    it_misses++;
    prev = GHB_INVALID_INDEX;
  ...
  if (ghb[ghb_head].stamp != 0)
    ghb_overwrites++;

  ghb[ghb_head].block_addr = block_addr;
  ghb[ghb_head].prev = prev;
  ghb[ghb_head].stamp = ghb_stamp++;

  itab[it_idx].valid = 1;
  itab[it_idx].pc_tag = pc;
  itab[it_idx].head = ghb_head;
  itab[it_idx].head_stamp = ghb[ghb_head].stamp;

  ghb_head++;
  if (ghb_head >= GHB_SIZE) ghb_head = 0;

  ghb_trains++;
  ghb_lookups++;

  int e1 = prev;
  if (e1 != GHB_INVALID_INDEX) {
    int e2 = ghb[e1].prev;
    if (e2 != GHB_INVALID_INDEX) {
      ghb_chain2_ok++;

      md_addr_t a0 = block_addr;
      md_addr_t a1 = ghb[e1].block_addr;
      md_addr_t a2 = ghb[e2].block_addr;

      md_addr_t s1 = (md_addr_t)(a0 - a1);
      md_addr_t s2 = (md_addr_t)(a1 - a2);
      ...
      if (s1 != 0 && s1 == s2) {
        ...
        if (cache_dl2) {
          ...
          int k;
          for (k = 1; k <= ghb_degree; k++) {
            md_addr_t pf_addr_k = (md_addr_t)(a0 + (md_addr_t)k * s1);
            ghb_pref_candidates++;

            (void)cache_access(cache_dl2, Read, pf_addr_k, NULL,
                             1, pt_cycle, NULL, NULL);
            ghb_pref_issued++;
            issued++;
          }
          ghb_stride_found++;
        } else {
          ghb_pref_skip_no_ul2++;
        }
        #endif
      }
    }
...
"latency of all pipeline operations.\n"
  ...
  opt_reg_int(odb, "-fastfwd", "number of insts skipped before timing starts",
	      ...
	      /* print */TRUE, /* format */NULL);
    /* ---------------- GHB prefetch options ---------------- */

  opt_reg_flag(odb, "-ghb:enable", "enable GHB stride prefetcher",
               &ghb_enable, /* default */0,
               /* print */TRUE, /* format */NULL);

  opt_reg_int(odb, "-ghb:degree", "GHB prefetch degree (#prefetches per trigger)",
              &ghb_degree, /* default */4,
              /* print */TRUE, /* format */NULL);
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  stat_reg_counter(sdb, "ghb_updates",
        "number of ul2 misses observed by GHB hook",   //ul2 = unified level-2 cache
        &ghb_updates, 0, NULL);
  stat_reg_counter(sdb, "ghb_trains",
        "GHB updates on ul2 demand misses",
        &ghb_trains, 0, NULL);
  stat_reg_counter(sdb, "it_hits",
        "Index table hits",
        &it_hits, 0, NULL);
  stat_reg_counter(sdb, "it_misses",
        "Index table misses",
        &it_misses, 0, NULL);
  stat_reg_counter(sdb, "ghb_overwrites",
        "GHB ring overwrites",
        &ghb_overwrites, 0, NULL);
  stat_reg_counter(sdb, "ghb_hook_misses",
        "number of ul2 misses observed by local GHB hook (store path)",
        &ghb_hook_misses, 0, NULL);
  stat_reg_counter(sdb, "ghb_lookups",
        "number of stride lookups after training (observe-only)",
        &ghb_lookups, 0, NULL);
  stat_reg_counter(sdb, "ghb_chain2_ok",
        "number of times the per-PC chain depth >= 2",
        &ghb_chain2_ok, 0, NULL);
  stat_reg_counter(sdb, "ghb_stride_found",
        "number of constant strides detected (observe-only)",
        &ghb_stride_found, 0, NULL);
  stat_reg_counter(sdb, "ghb_pref_candidates",
        "number of stride-based prefetch candidates (not issued)",
        &ghb_pref_candidates, 0, NULL);
  stat_reg_counter(sdb, "ghb_pref_issued",
        "number of stride-based prefetches issued to ul2",
        &ghb_pref_issued, 0, NULL);
  stat_reg_counter(sdb, "ghb_pref_skip_no_ul2",
        "number of times prefetch skipped because ul2 disabled",
        &ghb_pref_skip_no_ul2, 0, NULL);
      ...
      if (RUU[RUU_head].ea_comp)
	{
	  ...
	  if ((MD_OP_FLAGS(LSQ[LSQ_head].op) & (F_MEM|F_STORE))
	      ...
	      if (fu)
		{
          ...
          counter_t ul2_miss_before = (cache_dl2 ? cache_dl2->misses : 0);
		      lat =
			cache_access(cache_dl1, Write, (LSQ[LSQ_head].addr&~3),
				     NULL, 4, sim_cycle, NULL, NULL);
		      if (lat > cache_dl1_lat)
			events |= PEV_CACHEMISS;
          if (cache_dl2 && cache_dl2->misses != ul2_miss_before)
          {
            if (ghb_enable) {
              md_addr_t addr = (LSQ[LSQ_head].addr & ~3);
              md_addr_t block_addr = addr & ~((md_addr_t)(cache_dl2->bsize - 1));
              md_addr_t pc = rs->PC;

              ghb_hook_misses++;
              ghb_train(pc, block_addr);
            }
          }
		    }
